...
### 문제 풀이
<p>이전에 풀었던 LIS(가장 긴 증가하는 부분 수열) 문제와 똑같이 풀었다. 이 문제는 각 연결선의 양 끝에 물려있는 포트 번호가 둘 다 증가하는 방향이어야 교차하지 않게 된다. 이 조건은 문제 상에서 주어진 input이 왼쪽 포트 번호가 오름차순이므로 오른쪽 포트 번호만 오름차순이면 만족하게 된다. 따라서, 주어진 번호 arr의 LIS를 구하면 되고, 이는 N*logN 시간 복잡도(이분 탐색)를 가지며 n의 최댓값은 40000이므로 문제 없다. </p>
