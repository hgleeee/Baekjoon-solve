...
<p> 수식의 길이가 최대 19인 점을 착안하여 모든 연산 기호에 괄호를 넣거나 넣지 않는 경우의 수를 모두 고려하여 브루트포스 알고리즘을 적용하였고, 구현 방법은 연산을 할 때 숫자 arr를 왼쪽은 결과값, 오른쪽은 INF(도달할 수 없는) 값을 놓음으로써 어떤 순서로 하든 결과값이 arr의 첫 번째 index에 저장되도록 구현하였다. </p>
