...
### 문제 풀이
<p> 문제를 보자마자 떠오른 생각은 예전에 풀었었던 회의실 배정 문제였다. 회의실 배정같은 경우에는 끝나는 시간에 맞추어 정렬을 하고 그리디 알고리즘을 적용한 느낌이라면, 이 문제 같은 경우에는 빌리는 책 번호 범위의 끝 번호에 맞추어 정렬을 하되, 끝 번호가 같은 경우에는 앞 번호가 오름차순이 되도록 정렬이 필요했다.</p>
<p> 위와 같은 정렬이 필요한 이유는 끝 번호에 맞추어서 정렬을 하였을 때 앞에서부터 책 번호를 배정하는데 있어서 위상이 정렬되기 때문이다.</p>
